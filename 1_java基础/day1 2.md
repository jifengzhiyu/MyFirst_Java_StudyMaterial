...
- Java SE：java语言的标准版，其他两个版本的基础，桌面应用开发
- Java ME：java语言的小型版，嵌入式电子设备或者小型移动设备
- Java EE：java语言的企业版，企业应用，Web方向开发

稳定的常用的JDK：JDK8,JDK11,JDK17

# 字面量

- 直接写出来的人可以理解的数据，在java中叫做字面量。用来表示源代码中一个固定值。
- 分类：
  - 字符串字面量（双引号）
  - 整数字面量
  - 小数 字面量
...
- Java源文件编译可能产生不止一个二进制文件（取决于源文件类的个数）
- 编译javac，运行javaa
...
- 字符串常量池： 内会复用
  ...
  - 双引号创建的字符串对像，在字符串常量池中存储，通过构造方法创建的字符串对像，在堆内存中存储，每一次new都会申请一个内存空间，虽然内容相同，但是地址值不同
...
- String可以和8种基本数据类型变量做运算，且运算只能是连接运算：+
  ...
  - 运算的结果仍然是String类型
  - 当字符串变量使用+号串联（拼接）的时候（s1+"a"），系统底层会自动创建一个StringBuilder对像， 然后再调用其append方法完成拼接，拼接后，再调用其toString方法转换为String类型，存在堆中
  - 当字符串常量使用+号串联（拼接）的时候（"a"+"b”+"c"），Java存在常量优化机制，在编译的时候，就会将"a"+"b”+"c"拼接为"abc"
...
- String成员方法：
  ...
  - 截取：
    -  String substring(int beginlndex,int endlndex)，包含头，不包含尾
    -  String substring(int beginlndex)，从传入的索引位置处，向后截取，一直截取到末尾，得到新的字符串并返回，包含头
